    ...
    <div style="width: 300px;margin:0 auto;padding:10px 30px;color:#fff;display: none;" id="stop">
      ...
      <div style="width: 300px;background: #e7e7e7;margin:0 auto;padding:10px 30px;" >
        <form method="POST" action="{{ url('/register') }}">
            ...
            <p>实名：<input type="text" class="form-control" name="name" value="{{ old('name') }}"></p>

                    @if ($errors->has('name'))
                        <p style="color:#f30;">{{ $errors->first('name') }}</p>
                    @endif

            <p>邮箱：<input type="email" class="form-control" name="email" value="{{ old('email') }}"></p>
            ...
            <p>确认密码：<input type="password" class="form-control" name="password_confirmation"></p>

                    @if ($errors->has('password_confirmation'))
                       <p style="color:#f30;">{{ $errors->first('password_confirmation') }}</p>
                    @endif

            <p><button type="submit" style="border: none;cursor:pointer;padding: 10px 30px;width: 100%;color:#fff;background: #a40">
                注册
            </button></p>
